...
const fakeBackend = () => {
  ...
  mock.onPost(url.ADD_CARD_DATA).reply(cardData => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (cardData && cardData.data) {
          // Passing fake JSON data as response
          resolve([200, cardData.data])
        } else {
          reject([400, "Cannot add Card Data"])
        }
      })
    })
  })

  mock.onPut(url.UPDATE_CARD_DATA).reply(cardData => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (cardData && cardData.data) {
          // Passing fake JSON data as response
          ...
          reject([400, "Cannot update Card Data"])
        }
      })
    })
  })
